        ...
        {{ preselect }}
          <ul class="noListStyle">
            <li v-bind:key="j" v-for="(item, j) in leaders">
              <button v-if="!used.includes(item.id)" class="si-btn" :class="{'si-btn-off' : item.days[day] != true, 'si-btn-green' : preselect == item.id  }" v-on:click="handlerLeader(item)">{{ item.user.name }}</button>
            </li>
          </ul>
        </div>

...
class extGanztagsModelGroups
{
    ...
    public static function getByID($id = false)
    {
        ...
        $dataSQL = DB::getDB()->query_first("SELECT *  FROM ext_ganztags_groups WHERE id = ".(int)$id);
    ...
    {
        $ret = [];
        $dataSQL = DB::getDB()->query("SELECT *  FROM ext_ganztags_groups WHERE type = 'group' ");
        while ($data = DB::getDB()->fetch_array($dataSQL, true)) {
            $ret[] = new self($data);
        }
        return $ret;
